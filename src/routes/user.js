...
user.post('/user/registration', async (request , response)=>{
    ...
    let userResult = await User(user).save()
        .then(result=> {response.send(JSON.stringify(_.pick(result, ['name', 'email'])))}).catch(error=> response.send(JSON.stringify({error: error})))
})
...
user.post('/user/login', async (request, response)=>{
    ...
    if (!validPassword) return response.status(402).send(JSON.stringify({error:"Wrong Password"}))
    response.send(JSON.stringify(_.pick(user,'name', 'email')))
})
